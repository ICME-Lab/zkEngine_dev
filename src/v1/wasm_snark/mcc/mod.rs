    ...
    fn synthesize<CS: ConstraintSystem<F>>(
        ...
        for (i, (rs, ws)) in self.RS.iter().zip_eq(self.WS.iter()).enumerate() {
            ...
            let (r_addr, r_val, r_ts) =
                alloc_avt_tuple(cs.namespace(|| format!("rs{i}")), *rs)?;

            // (b) (a′,v′,wt) ← WS[i]
            let (w_addr, w_val, w_ts) =
                alloc_avt_tuple(cs.namespace(|| format!("ws{i}")), *ws)?;

            // (c) gts ← gts + 1
            gts = add(
                cs.namespace(|| format!("{i},  gts ← gts + 1")),
                &gts,
                &one,
            )?;
    ...
    F: PrimeField,
{
    ...
    fn synthesize<CS: ConstraintSystem<F>>(
        ...
        for (i, (is, fs)) in self.IS.iter().zip_eq(self.FS.iter()).enumerate() {
            // (a) (a,v,it)←IS[i]
            let (i_addr, i_val, i_ts) = alloc_avt_tuple(
                cs.namespace(|| format!("is{i}")),
                (i, is.0, is.1),
            )?;

            // (b) (a′,v′,ft) ← FS[i]
            let (f_addr, f_val, f_ts) = alloc_avt_tuple(
                cs.namespace(|| format!("fs{i}")),
                (i, fs.0, fs.1),
            )?;
