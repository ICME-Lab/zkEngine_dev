  ...
  CS: ConstraintSystem<F>,
{
  ...
  let c_128 =
    SwitchBoardCircuit::alloc_num(&mut cs, || "c_128", || Ok(F::from_u128(c_128_bits)), switch)?;
  ...
  // c_intermediate == c_128
  cs.enforce(
    || "c_intermediate == c",
    |lc| lc + c_intermediate.get_variable(),
    |lc| lc + CS::one(),
    |lc| lc + c_128.get_variable(),
  );
  ...
  cs.enforce(
    ...
    |lc| lc + c.get_variable(),
    |lc| lc + CS::one(),
    |lc| lc + c_128.get_variable() - left_overs.get_variable(),
  );
